...
from GraphMetricsIndividual import create_graph_list_individual, open_features_dict
...
# Initialize W&B
wandb.init(
    project="GCN-Contrastive-Training",  # Replace with your project name
...
class GCN(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels, num_classes):
        ...
        self.conv2 = GCNConv(hidden_channels//2, hidden_channels)# Add more layers and complexity
        self.conv3 = GCNConv(hidden_channels, hidden_channels*2)
        self.conv4 = GCNConv(hidden_channels, hidden_channels//4)
        self.lin1 = torch.nn.Linear(hidden_channels*304, hidden_channels//4)
        self.lin2 = torch.nn.Linear(hidden_channels//4, num_classes)
        self.batch_norm1 = torch.nn.BatchNorm1d(hidden_channels//4)
        self.batch_norm2 = torch.nn.BatchNorm1d(hidden_channels//4)
        self.norm1 = GraphNorm(hidden_channels//2)
        self.norm2 = GraphNorm(hidden_channels)
        self.norm3 = GraphNorm(hidden_channels*2)
        self.norm4 = GraphNorm(hidden_channels//4)
        self.norm5 = GraphNorm(hidden_channels//4)
    ...
    def forward(self, x, edge_index, batch=None):
        ...
        x = torch.cat((x_mean, x_max), dim=1)  # [batch_size, 2 * hidden_channels]
...
def create_gcn_dataset(graphlist, features_dict):
    ...
    for graph, mstype, edds, name in graphlist:

        convert_types = {-1:0, 0:1, 1:2, 2:3}
        ...
        for col in range(node_features.shape[1]):  # Iterate through the 7 columns
            col_min = np.min(node_features[:, col])
            col_max = np.max(node_features[:, col])

            # Normalize the column
            if col_max - col_min == 0:
                # If all values are the same, set to 0
                node_features[:, col] = 0.0
            else:
                # Normalize to range [0, 1]
        
                node_features[:, col] = (node_features[:, col] - col_min) / (col_max - col_min + 1e-8)
...
def train_gcn_model(model, train_loader, val_loader, num_epochs=20, learning_rate=0.001):
    ...
    criterion = torch.nn.CrossEntropyLoss(weight=torch.tensor([1.75, 9, 20, 4], dtype=torch.float).to(device)) # Adjust weights as needed
    scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=2000, T_mult=2, eta_min=0.00001)
    ...
    for epoch in range(num_epochs):
        ...
        for data in train_loader:
            ...
            triplet_loss = online_batch_ordering(embedding, data.y, device)
            classification_loss = criterion(out, data.y)
            loss = classification_loss.to(device) + 0.1*triplet_loss
            loss.backward()
        ...
        train_recall = recall_score((data.y).cpu(), predicted.cpu(), labels=[1], average=None, zero_division=0)[0]
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                triplet_loss = online_batch_ordering(embedding, data.y, device)
                classification_loss = criterion(out, data.y)
                loss = classification_loss + 0.1*triplet_loss
                val_loss += loss.item()
                _, predicted = out.max(1)
                total += data.y.size(0)
                correct += predicted.eq(data.y).sum().item()
        ...
        val_recall = recall_score((data.y).cpu(), predicted.cpu(), labels=[1], average=None, zero_division=0)[0]
        # Log metrics to W&B
        wandb.log({
            "epoch": epoch + 1,
        ...
        print(f'Epoch {epoch+1}/{num_epochs}: '
              ...
              f'Train Recall Score: {train_recall:.4f}, '
              f'Val Loss: {val_loss/len(val_loader):.4f}, '
              f'Val Acc: {val_acc:.2f}%,'
              f'Val Recall Score: {val_recall:.4f}')
        # Update the learning rate
        
        # Save the best model
        if (val_acc > best_val_acc) & (float(val_loss) < min_val_loss):
...
def test_gcn_model(model, test_loader):
    ...
    for class_label in sorted(set(all_y)):
        ...
        print(f"Class {class_label}: Accuracy:{accuracy:.4f} Precision: {class_precision:.4f}, Recall: {class_recall:.4f}, F1 Score: {class_f1:.4f}")
...
# Example usage
    ...
    hidden_channels = 128  # Number of hidden channels in GCN layers
    ...
    trained_model = train_gcn_model(model, train_loader, val_loader, num_epochs=1000, learning_rate = 0.0001)
