...
class GCN(torch.nn.Module):
    def __init__(self, num_node_features, hidden_channels, num_classes, node_dropout=0.1, edge_dropout=0.2):
...
def create_gcn_dataset(graphlist, features_dict):
    ...
    for graph, mstype, edds, name, dataset_name in graphlist:
        ...
        y = torch.tensor(names_dataset, dtype=torch.long).view(1)
...
def train_gcn_model(model, train_loader, val_loader, num_epochs=20, learning_rate=0.001):
    ...
    criterion = torch.nn.CrossEntropyLoss()  # Adjust weights as needed weight=torch.tensor([1.75, 9, 20, 4], dtype=torch.float).to(device)
...
if __name__ == "__main__":
    ...
    unique_labels = set()
    '''for _, mstype, _, _, dataset_name in graphlist_train:
        unique_labels.add(mstype)
    num_classes = len(unique_labels)'''
    num_classes = 2  # Assuming 2 classes as per the dataset
