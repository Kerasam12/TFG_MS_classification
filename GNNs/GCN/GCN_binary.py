...
if __name__ == "__main__":
    ...
    dataset_train = create_gcn_dataset(graphlist_train, features_dict_train, convert_types = {-1:0, 0:1, 1:1, 2:1})
    print(f"Training samples: {len(dataset_train)}")

    dataset_test = create_gcn_dataset(graphlist_test, features_dict_test, convert_types = {-1:0, 0:1, 1:1, 2:1})
    ...
    best_model_path = 'best_gcn_model_binary.pth'
    model = GCN_dropout(num_node_features, hidden_channels, num_classes, node_dropout=0.2, edge_dropout=0.2)
    trained_model = train_gcn_model(model, train_loader, val_loader, num_epochs=5000, learning_rate = 0.00001, model_path = best_model_path, weight= torch.tensor([0.80, 0.2]))
