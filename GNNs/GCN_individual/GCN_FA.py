...
from GNNs.GCN.models.GCN_models_individual import GCN_dropout
...
from Data_Preprocessing.GraphMetricsXMatrix import create_graph_list_x_matrix
...
route_features_train = "/home/samper12/Escritorio/IA carrera/TFG/codi_TFG/graphs/graphs_dict_train_FA/"
route_features_test = "/home/samper12/Escritorio/IA carrera/TFG/codi_TFG/graphs/graphs_dict_test_FA/"   
...
if __name__ == "__main__":
    ...
    graphlist_train = create_graph_list_x_matrix(datalist_augmented)
    graphlist_test = create_graph_list_x_matrix(datalist_test)

    graphlist_train = [ (g[0], mstype, edds, name, dataset_name) for g, mstype, edds, name, dataset_name in graphlist_train if g is not None]
    graphlist_test = [ (g[0], mstype, edds, name, dataset_name) for g, mstype, edds, name, dataset_name in graphlist_test if g is not None]

    # 2. Load features from disk
    features_dict_train = open_features_dict("/home/samper12/Escritorio/IA carrera/TFG/codi_TFG/features/featues_dict_train_FA")
    features_dict_test = open_features_dict("/home/samper12/Escritorio/IA carrera/TFG/codi_TFG/features/featues_dict_test_FA")
    ...
    model = GCN_dropout(num_node_features, hidden_channels, num_classes)
    best_model_path = 'best_gcn_model_FA.pth'
    trained_model = train_gcn_model(model, train_loader, val_loader, num_epochs=2000, learning_rate = 0.0005, model_path = best_model_path, weight = torch.tensor([1.75, 9, 20, 4]))
    

    
    best_trained_model = GCN_dropout(num_node_features, hidden_channels, num_classes)
    ...
    explain_model(best_trained_model, test_loader, heatmap_name="feature_importance_heatmap_FA.png")
