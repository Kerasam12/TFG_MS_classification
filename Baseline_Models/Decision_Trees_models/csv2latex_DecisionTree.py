...
# Save as .tex file
tex_filename = f"visualization_output/trees_main_results.tex"
with open(tex_filename, "w") as f:
    f.write("\\begin{table}[htbp]\n")
    f.write("\\centering\n")
    f.write("\\caption{Main Results for Trees}\n")
    f.write("\\label{tab:trees_main_" "}\n")
    f.write(latex_table)
    f.write("\\end{table}\n")
...
if all(col in df.columns for col in accuracy_cols):
    ...
    # Save as .tex file
    tex_filename = f"visualization_output/trees_class_accuracy.tex"
    with open(tex_filename, "w") as f:
        f.write("\\begin{table}[htbp]\n")
        f.write("\\centering\n")
        f.write("\\caption{Per-Class Accuracy for Trees with}\n")
        f.write("\\label{tab:trees_accuracy\n")
        f.write(latex_table)
        f.write("\\end{table}\n")
...
if all(col in df.columns for col in precision_cols) and all(col in df.columns for col in recall_cols):
    ...
    tex_filename = f"visualization_output/trees_class_precision.tex"
    with open(tex_filename, "w") as f:
        f.write("\\begin{table}[htbp]\n")
        f.write("\\centering\n")
        f.write("\\caption{Per-Class Precision for Tress with \n")
        f.write("\\label{tab:trees_precision\n")
        f.write(latex_table)
        f.write("\\end{table}\n")
    ...
    tex_filename = f"visualization_output/trees_class_recall.tex"
    with open(tex_filename, "w") as f:
        f.write("\\begin{table}[htbp]\n")
        f.write("\\centering\n")
        f.write("\\caption{Per-Class Recall for trees with}\n")
        f.write("\\label{tab:trees_recall_" "}\n")
        f.write(latex_table)
        f.write("\\end{table}\n")
...
if 'Threshold' in df.columns and 'Mixing Level' in df.columns and 'Mean Accuracy' in df.columns:
    ...
    plt.title(f'Trees: Accuracy by Threshold and Mixing Level')
    plt.tight_layout()
    plt.savefig(f"visualization_output/trees_heatmap.png", dpi=300)
    plt.close()
    
    # Generate TikZ code for LaTeX
    tikz_filename = f"visualization_output/trees_heatmap.tex"
    with open(tikz_filename, "w") as f:
        f.write("\\begin{figure}[htbp]\n")
        f.write("\\centering\n")
        f.write("\\includegraphics[width=0.8\\textwidth]{" + f"visualization_output/trees_heatmap.png" + "}\n")
        f.write("\\caption{Heatmap of Mean Accuracy for Trees with ""}\n")
        f.write("\\label{fig:trees_heatmap_" "}\n")
        f.write("\\end{figure}\n")
...
if 'Mean Accuracy' in df.columns:
    ...
    if all(col in df.columns for col in accuracy_cols + precision_cols + recall_cols):
        ...
        ax.set_ylabel('Score')
        ax.set_title(f'Best Model Performance by Class (Trees )')
        ax.set_xticks(x)
        ...
        plt.tight_layout()
        plt.savefig(f"visualization_output/trees_class_comparison.png", dpi=300)
        plt.close()
        
        # Generate LaTeX figure
        fig_filename = f"visualization_output/trees_class_comparison.tex"
        with open(fig_filename, "w") as f:
            f.write("\\begin{figure}[htbp]\n")
            f.write("\\centering\n")
            f.write("\\includegraphics[width=0.9\\textwidth]{" + f"visualization_output/trees_class_comparison.png" + "}\n")
            f.write(f"\\caption{{Performance by Class for Best tree Model (Threshold: {best_config['Threshold']}, Mixing Level: {best_config['Mixing Level']})}}\n")
            f.write("\\label{fig:trees_class_comp_"  "}\n")
            f.write("\\end{figure}\n")

print(f"Completed visualization - check the 'visualization_output' directory")
