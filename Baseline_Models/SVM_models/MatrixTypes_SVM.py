    ...
    for idx_matrix, matrix_type in enumerate(matrix_type):
        ...
        for kernel in kernels:
            ...
            for prunning_threshold in thresholds:
                ...
                mixed_datalist = datalist_mixing_balanced(datalist_train, mix, 5000)
                ...
                X_train, y_train = generate_train_test(datalist_single_matrix_train, test_size=0)
                X_test, y_test= generate_train_test(datalist_single_matrix_test, test_size=0)
                ...
                # Append results
                results.append({
                    "Samples  per class": len(X_train)/4,
            ...
            filename = f"svm_full_results_{matrix_type}_{kernel}.csv"
            df_results.to_csv(filename, index=False)
            print(f"Saved results for kernel: {kernel} -> {filename}")
