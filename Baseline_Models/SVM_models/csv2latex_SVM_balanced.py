...
for kernel in kernels:
    # Load CSV file
    filename = f"svm_full_results_balanced_{kernel}.csv"
    df = pd.read_csv(filename)
    
    # Create directory for outputs
    os.makedirs("visualization_output_balanced", exist_ok=True)
    ...
    # Save as .tex file
    tex_filename = f"visualization_output_balanced/svm_main_results_balanced_{kernel}.tex"
    ...
    if all(col in df.columns for col in accuracy_cols):
        ...
        # Save as .tex file
        tex_filename = f"visualization_output_balanced/svm_class_accuracy_balanced_{kernel}.tex"
        with open(tex_filename, "w") as f:
            f.write("\\begin{table}[htbp]\n")
            f.write("\\centering\n")
    ...
    if all(col in df.columns for col in precision_cols) and all(col in df.columns for col in recall_cols):
        ...
        tex_filename = f"visualization_output_balanced/svm_class_precision_balanced_{kernel}.tex"
        with open(tex_filename, "w") as f:
            f.write("\\begin{table}[htbp]\n")
            f.write("\\centering\n")
        ...
        tex_filename = f"visualization_output_balanced/svm_class_recall_balanced_{kernel}.tex"
        with open(tex_filename, "w") as f:
            f.write("\\begin{table}[htbp]\n")
            f.write("\\centering\n")
    ...
    if 'Threshold' in df.columns and 'Mixing Level' in df.columns and 'Mean Accuracy' in df.columns:
        ...
        plt.tight_layout()
        plt.savefig(f"visualization_output_balanced/svm_heatmap_balanced_{kernel}.png", dpi=300)
        plt.close()
        
        # Generate TikZ code for LaTeX
        tikz_filename = f"visualization_output_balanced/svm_heatmap_balanced_{kernel}.tex"
        with open(tikz_filename, "w") as f:
            f.write("\\begin{figure}[htbp]\n")
            f.write("\\centering\n")
            f.write("\\includegraphics[width=0.8\\textwidth]{" + f"visualization_output_balanced/svm_heatmap_balanced_{kernel}.png" + "}\n")
    ...
    if 'Mean Accuracy' in df.columns:
        ...
        if all(col in df.columns for col in accuracy_cols + precision_cols + recall_cols):
            ...
            plt.tight_layout()
            plt.savefig(f"visualization_output_balanced/svm_class_comparison_balanced_{kernel}.png", dpi=300)
            plt.close()
            
            # Generate LaTeX figure
            fig_filename = f"visualization_output_balanced/svm_class_comparison_balanced_{kernel}.tex"
            with open(fig_filename, "w") as f:
                f.write("\\begin{figure}[htbp]\n")
                f.write("\\centering\n")
                f.write("\\includegraphics[width=0.9\\textwidth]{" + f"visualization_output_balanced/svm_class_comparison_balanced_{kernel}.png" + "}\n")
    ...
    print(f"Completed visualization for {kernel} kernel balanced - check the 'visualization_output_balanced' directory")
