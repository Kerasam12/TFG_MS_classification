...
def create_graph_list_individual(datalist):
    """
    ...
    for datapoint in datalist:
        ...
        for idx_node in range(Func_matrix.shape[0] + GM_matrix.shape[0]):
            
            G.add_node(str(idx_node))
        
        # Add edges of each matrix
        for idx,matrix in enumerate((Func_matrix, GM_matrix)):
...
def compute_metrics(graphlist, features_dict):
    ...
    for idx, datapoint in enumerate(graphlist):
        ...
        features_dict[name] = np.zeros((152,161))
    ...
    for idx,datapoint in enumerate(graphlist):
        ...
        for node in graph.nodes():
            ...
            features_node = np.zeros(161)
            features_node[:8] = [node_degree, node_strength, triangles, closeness_centrality, node_betweenness_centrality, clustering, local_eff, pagerank[node]]
            
            for node in graph.nodes():
                name_sp = 'shortest_path' + str(node)
                graph.nodes[node][name_sp] = len(shortest_path[node])
            
                features_node[int(node) + 8] = len(shortest_path[node])
...
def save_graphlist(graphlist, train = True):
    ...
    if train:
        filepath = "computed_graphs_train_shortest_path"
        if not os.path.exists("computed_graphs_train_shortest_path"):
            os.makedirs("computed_graphs_train_shortest_path")
    else:
        filepath = "computed_graphs_test_shortest_path"
        if not os.path.exists("computed_graphs_test_shortest_path"):
            os.makedirs("computed_graphs_test_shortest_path")
...
def save_features(features_dict, train = True):
    ...
    """
    if train:
        filepath = "computed_features_train_shortest_path/features_dict.pkl"
        # Create directory if it doesn't exist
        if not os.path.exists("computed_features_train_shortest_path"):
            os.makedirs("computed_features_train_shortest_path")
    else:
        filepath = "computed_features_test_shortest_path/features_dict.pkl"
        # Create directory if it doesn't exist
        if not os.path.exists("computed_features_test_shortest_path"):
            os.makedirs("computed_features_test_shortest_path")
...
def open_features_dict(features_dict_path, train = True):
    """
...
if __name__ == "__main__":
    ...
    graphlist_train= create_graph_list_individual(datalist_augmented_train)
