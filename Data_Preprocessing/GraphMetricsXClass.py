...
def create_graph_list_x_class(datalist):
    """
    ...
    for datapoint in datalist:
        ...
        for idx_node in range(Func_matrix.shape[0]):
            G_fMRI.add_node(str(idx_node))
        
        for idx_node in range(GM_matrix.shape[0]):
            G_GM.add_node(str(idx_node))


        for idx, matrix in enumerate(FA_matrix):
            for i in range(matrix.shape[0]):
                for j in range(matrix.shape[1]):
                    if matrix[i,j] != 0 or i != j:
                        ...
                        node1 = str(i)
                        node2 = str(j)
                        G_FA.add_edge(node1, node2, weight=matrix[i,j])
        for idx, matrix in enumerate(Func_matrix):
            for i in range(matrix.shape[0]):
                for j in range(matrix.shape[1]):
                    if matrix[i,j] != 0 or i != j:
                        ...
                        node1 = str(i)
                        node2 = str(j)
                        G_fMRI.add_edge(node1, node2, weight=matrix[i,j])
        for idx, matrix in enumerate(GM_matrix):
            for i in range(matrix.shape[0]):
                for j in range(matrix.shape[1]):
                    if matrix[i,j] != 0 or i != j:
                        ...
                        node1 = str(i)
                        node2 = str(j)
                        G_GM.add_edge(node1, node2, weight=matrix[i,j])
                        
        # Append graph to list
        graph_datalist_FA.append((G_FA, mstype, edds, name))
        graph_datalist_fMRI.append((G_fMRI, mstype, edds, name))
        graph_datalist_GM.append((G_GM, mstype, edds, name))

    return graph_datalist_FA, graph_datalist_fMRI, graph_datalist_GM
...
def compute_metrics(graphlist, features_dict):
    ...
    for idx, datapoint in enumerate(graphlist):
        ...
        features_dict[name] = np.zeros((76,7))
...
def save_features(features_dict, train = True):
    ...
    """
    if train:
        filepath = "computed_features_train_individual/features_dict.pkl"
        # Create directory if it doesn't exist
        if not os.path.exists("computed_features_train_individual"):
            os.makedirs("computed_features_train_individual")
    else:
        filepath = "computed_features_test_individual/features_dict.pkl"
        # Create directory if it doesn't exist
        if not os.path.exists("computed_features_test_individual"):
            os.makedirs("computed_features_test_individual")
...
if __name__ == "__main__":
    ...
    graphlist_train_fa, graphlsit_train_fmri, graphlsit_train_gm= create_graph_list_x_class(datalist_augmented_train)
    graphlist_test_fa, graphlsit_test_fmri, graphlsit_test_gm = create_graph_list_x_class(datalist_test)
    features_dict_train = {}
