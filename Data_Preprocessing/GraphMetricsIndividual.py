...
def create_graph_list_individual(datalist):
    ...
    for datapoint in datalist:
        ...
        graph_datalist.append((G, mstype, edds, name, dataset_name))
...
def compute_metrics(graphlist, features_dict):
    ...
    for idx, datapoint in enumerate(graphlist):
        ...
        features_dict[name] = np.zeros((152,9))
...
def save_graphlist(graphlist, train = True):
    ...
    if train:
        filepath = "computed_graphs_train_individual"
        if not os.path.exists("computed_graphs_train_individual"):
            os.makedirs("computed_graphs_train_individual")
    else:
        filepath = "computed_graphs_test_individual"
        if not os.path.exists("computed_graphs_test_individual"):
            os.makedirs("computed_graphs_test_individual")
